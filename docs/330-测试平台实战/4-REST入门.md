...
[Django REST framework](https://www.django-rest-framework.org/)（以下简称 DRF 或 REST 框架）是一个开源的 Django 扩展，提供了便捷的 REST API 开发框架，拥有以下特性：
...
除了序列化模型实例，序列化器还可以序列化查询结果集（querysets），只需要为 serializer 添加一个
`many=True` 标志。
...
可用的 Meta 选项：[模型 Meta 选项 | Django 文档 | Django (djangoproject.com) ](https://docs.djangoproject.com/zh-hans/3.2/ref/models/options/)
